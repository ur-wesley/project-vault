...
import { getDirSizeBreakdown, getDirSizeTree } from "~/services/tauri/projects";
import { ProjectDiskUsageTreemap } from "./ProjectDiskUsageTreemap";

type Crumb = { name: string; path: string };
type DiskUsageView = "list" | "treemap";
...
export function ProjectDiskUsageDialog(props: {
  ...
  const [view, setView] = createSignal<DiskUsageView>("list");
  ...
  const q = createQuery(() => ({
    ...
    enabled: props.open && view() === "list" && currentPath().length > 0,
  }));

  const treeQuery = createQuery(() => ({
    queryKey: ["dir-size-tree", props.projectPath],
    queryFn: async () => {
      const r = await getDirSizeTree(props.projectPath);
      if (r.isErr()) throw new Error(r.error.message);
      return r.value;
    },
    enabled: props.open && view() === "treemap" && props.projectPath.length > 0,
  }));
  ...
  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent class={view() === "treemap" ? "max-w-2xl gap-3" : "max-w-lg gap-3"}>
        <DialogHeader>
          <DialogTitle class="text-sm font-bold">{props.projectName}</DialogTitle>
        </DialogHeader>

        <div class="flex items-center gap-1 rounded-md bg-muted/40 p-0.5 text-[11px]">
          <button
            type="button"
            class={cn(
          ...
          >
            <span class="iconify mdi--format-list-bulleted size-3.5" />
            {t("library.diskUsageListView") as string}
          </button>
          <button
            type="button"
            class={cn(
          ...
          >
            <span class="iconify mdi--chart-box-outline size-3.5" />
            {t("library.diskUsageTreemapView") as string}
          </button>
        </div>

        <Show when={view() === "treemap"}>
          <Show when={treeQuery.isPending}>
          ...
          <Show when={treeQuery.isSuccess && treeQuery.data}>
            {(data) => (
              ...
              >
                <ProjectDiskUsageTreemap root={data()} />
              </Show>
            )}
          </Show>
        </Show>
        ...
        <Show when={view() === "list"}>
          ...
          <div class="rounded-md border border-border/40 bg-muted/20 px-3 py-2 text-[11px]">
            <div class="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
              <div>
                <span class="text-muted-foreground">
                  {t("library.diskUsageOnDisk") as string}:{" "}
                </span>
              ...
              <Show when={props.projectSizeBytes > 0}>
                <div>
                  <span class="text-muted-foreground">
                    {t("library.diskUsageProjectSize") as string}:{" "}
                  </span>
                  <span class="font-mono font-medium tabular-nums">
            ...
            <p class="mt-1 text-[10px] leading-snug text-muted-foreground/80">
              <Show when={crumbs().length === 1}>{t("library.diskUsageSkipNote") as string}</Show>
            </p>
          </div>
              ...
              {(data) => (
                ...
                >
                  <For each={data().entries}>
                    {(entry) => {
                      const pct =
                        data().totalBytes > 0 ? (entry.sizeBytes / data().totalBytes) * 100 : 0;
                      return (
                        <Show
                          when={entry.isDir}
                          fallback={
                              ...
                              <div class="min-w-0 flex-1">
                                ...
                                <div class="mt-1 h-1 overflow-hidden rounded-full bg-muted/60">
                                  <div
                                    ...
                                    style={{
                                      width: `${Math.max(pct, entry.sizeBytes > 0 ? 2 : 0)}%`,
                                    }}
                                  />
                                </div>
                              </div>

...
pub fn register(lua: &Lua, vault: &Table, ctx: &ModuleContext) -> Result<()> {
    ...
    if let Some(app) = ctx.app.clone() {
        ...
        settings.set(
            ...
            lua.create_async_function(move |lua, key: String| {
                ...
                async move {
                    ...
                    crate::db::get_setting(&pool, &scoped_key)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))
                }
            })?,
            ...
            lua.create_async_function(move |lua, (key, value): (String, String)| {
                ...
                async move {
                    ...
                    let pool = crate::db::sqlite_pool(&*db)
                        ...
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let scoped_key = plugin_scoped_key(plugin_id.as_deref(), &key);
                    crate::db::set_setting(&pool, &scoped_key, &value)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))
                }
            })?,
        ...
        settings.set(
            "get",
            lua.create_function(|_, _key: String| Ok(None::<String>))?,
        )?;
        settings.set(
            "set",
            lua.create_function(|_, (_key, _value): (String, String)| Ok(()))?,
        )?;
        settings.set(
            "get_app",
            lua.create_function(|_, _key: String| Ok(None::<String>))?,

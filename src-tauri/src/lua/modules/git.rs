    ...
    if let Some(app) = ctx.app.clone() {
        ...
        git_mod.set(
            ...
            lua.create_async_function(
                move |lua, (project_id, args_val): (String, mlua::Value)| {
        ...
        let app_git_stats = app.clone();
        git_mod.set(
            "get_statuses",
            lua.create_async_function(move |_, project_ids: Vec<String>| {
                let app = app_git_stats.clone();
                async move {
                    let db = app.state::<tauri_plugin_sql::DbInstances>();
                    let res = crate::commands::git::status::batch_git_statuses(&*db, project_ids)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let json = serde_json::to_string(&res).map_err(mlua::Error::external)?;
                    Ok(json)
        ...
        git_mod.set(
            ...
            lua.create_async_function(move |_, project_id: String| {
                ...
                async move {
                    ...
                    if let Some(revs) =
                        run_cmd(&["rev-list", "--left-right", "--count", "HEAD...@{u}"])
                    {
                        has_upstream = true;
        ...
        git_mod.set(
            "run",
            lua.create_function(|lua, _: (String, mlua::Value)| {
        ...
        git_mod.set(
            "get_statuses",
            lua.create_function(|_, _: Vec<String>| Ok("[]".to_string()))?,
        )?;
        git_mod.set(
            "log",
            lua.create_function(|_, _: (String, Option<u32>)| Ok("[]".to_string()))?,
        )?;
        git_mod.set(
            "status",
            lua.create_function(|_, _: String| Ok(None::<String>))?,
        )?;
    }

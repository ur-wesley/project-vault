    ...
    if let Some(app) = ctx.app.clone() {
        ...
        shell.set(
            ...
            lua.create_async_function(
                move |_, (project_id, cwd_rel): (String, Option<String>)| {
                    let app = app_c.clone();
                    async move {
                        ...
                        if !project_path.is_dir() {
                            return Err(mlua::Error::RuntimeError(
                                "project path not a directory".to_string(),
                            ));
                        }
        ...
        shell.set(
            "spawn_process",
            lua.create_function(
                |_, (command, args, cwd): (String, Option<Vec<String>>, Option<String>)| {
        ...
        shell.set(
            "spawn_task",
            lua.create_async_function(
                move |_, (project_id, argv, cwd_rel): (String, Vec<String>, Option<String>)| {
                    let app = app_c.clone();
                    async move {
                        ...
                        let pool = crate::db::sqlite_pool(&*db_state)
                            .await
                            .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                        let project = crate::db::get_project(&pool, &project_id)
                            .await
                        ...
                        let session = crate::db::start_session(
                            &pool,
                            &project_id,
                            Some(cmd_line.clone()),
                            None,
        ...
        shell.set(
            "execute",
            lua.create_function(
                |lua, (command, args, cwd): (String, Option<Vec<String>>, Option<String>)| {
                    let mut cmd = std::process::Command::new(command);
                    if let Some(a) = args {
                        cmd.args(a);
                    }
        ...
        shell.set(
            "spawn_process",
            lua.create_function(|_, _: (String, Option<Vec<String>>, Option<String>)| Ok(0u32))?,
        )?;

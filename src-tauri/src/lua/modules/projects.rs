    ...
    if let Some(app) = ctx.app.clone() {
        ...
        projects.set(
            ...
            lua.create_async_function(move |_, _: ()| {
                ...
                async move {
                    ...
                    let list = crate::db::list_projects(&pool)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
            ...
            lua.create_async_function(move |_, project_id: String| {
                ...
                async move {
                    ...
                    let pool = crate::db::sqlite_pool(&*db)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let p = crate::db::get_project(&pool, &project_id)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
        ...
        projects.set(
            ...
            lua.create_async_function(
                move |lua,
                      (project_id, argv_val, cwd, concurrent_val): (
                    String,
                    mlua::Value,
                    Option<String>,
                    Option<mlua::Value>,
                    ...
                    let concurrent: Option<Vec<crate::models::ConcurrentTask>> =
                        concurrent_val.and_then(|v| lua.from_value(v).ok());
                    async move {
                        let db = app.state::<tauri_plugin_sql::DbInstances>();
                        let payload = crate::commands::task_runner::SpawnProjectTaskPayload {
                            project_id,
            ...
            lua.create_async_function(move |_, project_id: String| {
                ...
                async move {
                    ...
                    let pool = crate::db::sqlite_pool(&*db)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let list = crate::db::list_active_sessions_for_project(&pool, &project_id)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let json = serde_json::to_string(&list).map_err(mlua::Error::external)?;
                    Ok(json)
        ...
        projects.set(
            ...
            lua.create_async_function(move |_, project_id: String| {
                ...
                async move {
                    let db = app.state::<tauri_plugin_sql::DbInstances>();
                    let res = crate::commands::git::get_git_status(db, project_id)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
            ...
            lua.create_async_function(move |_, project_id: String| {
                ...
                async move {
                    ...
                    let pool = crate::db::sqlite_pool(&*db)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
                    let sessions = crate::db::list_sessions_for_project(&pool, &project_id, 100, 0)
                        .await
                        .map_err(|e| mlua::Error::RuntimeError(e.message))?;
        ...
        projects.set(
            "run_argv",
            lua.create_function(
                |_, _: (String, mlua::Value, Option<String>, Option<mlua::Value>)| Ok(()),
            )?,
        )?;
        ...
        projects.set(
            "set_favorite",
            lua.create_function(|_, _: (String, bool)| Ok(()))?,
        )?;
    }

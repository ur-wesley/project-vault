...
pub async fn sync_issues_from_commits(
    project_path: &str,
    project_id: &str,
    ...
    let output = Command::new("git")
        ...
        .output()
        .map_err(|e| {
            crate::error::StableError::new(codes::INTERNAL, format!("Failed to run git log: {}", e))
        })?;
...
pub async fn handle_cli_matches(matches: Matches, pool: Pool<Sqlite>) -> Result<()> {
    if let Some(sub) = matches.subcommand {
        ...
        let cwd = env::current_dir().map_err(|e| {
            crate::error::StableError::new(codes::INTERNAL, format!("Failed to get CWD: {}", e))
        })?;
        ...
        if let Some(issue_sub) = issue_matches.subcommand {
            match issue_sub.name.as_str() {
                ...
                "view" => {
                    ...
                    let number_str = view_matches
                        .args
                        .get("number")
                        .and_then(|a| a.value.as_str())
                        .unwrap_or_default();
                    let number = number_str.parse::<i64>().map_err(|_| {
                        crate::error::StableError::new(codes::INTERNAL, "Invalid issue number")
                    })?;
                ...
                "edit" => {
                    ...
                    let title = edit_matches
                        .args
                        .get("title")
                        .and_then(|a| a.value.as_str())
                        .map(|s| s.to_string());
                    let body = edit_matches
                        .args
                        .get("body")
                        .and_then(|a| a.value.as_str())
                        .map(|s| s.to_string());
